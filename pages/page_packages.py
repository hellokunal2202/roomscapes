    ...
    <div class="product-card">
        ...
        <div>
            <div class="product-title">
                <a href="{url}" target="_blank" style="text-decoration: none;" title="View Product: {name}">
                    {name}
                </a>
            </div>
            <div class="product-description">{description}</div>
            <div class="product-category">{category.replace("-", " ").title()}</div>
        </div>
        <div class="price-tag" style="background: linear-gradient(90deg, #667eea 0%, #764ba2 100%);">{formatted_price}</div>
    </div>
    """
...
if "package_summary" not in st.session_state or not st.session_state.package_summary:
    ...
    <div style="background: rgba(220, 53, 69, 0.1); padding: 20px; border-radius: 12px; border-left: 4px solid #dc3545; margin: 20px 0;">
        <p style="color: #f9fafb; margin-bottom: 0;">Please generate a design plan from the Preferences page first.</p>
    </div>
    """, unsafe_allow_html=True)
